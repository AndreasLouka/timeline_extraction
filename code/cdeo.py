...
def getTrainFeatures(ncorpus):

    corpus = ncorpus
    inFileEntity = '../data/evaluation/corpus_%s/list_target_entities_corpus_%s.txt' %(corpus, corpus)
    inDirCAT= '../data/evaluation/corpus_%s/corpus_trackA_CAT' %(corpus)
    collection = utils.loadDocs(inDirCAT)

    targetEntityList = utils.loadEntityList(inFileEntity)

    GoldEventMentions = AllEventExtraction.getGoldEventMentions(corpus, targetEntityList)

    Features, Features_array, Vectorizer, Labels = AllEventExtraction.getFeaturesTrain(corpus, targetEntityList, GoldEventMentions, collection)

    Classifier = AllEventExtraction.getTrainedClassifier(Features_array, Labels)

    return Classifier, Vectorizer
    

def getCollection(ncorpus, trained_classifier, Vectorizer):
    print '\n=============================================================='
    ...
    targetEntityList = utils.loadEntityList(inFileEntity)
    collection = utils.loadDocs(inDirCAT)
    linksGold = utils.goldTimelineList(targetEntityList, goldTimelineDir)
    doc_Event_Precision = list()
    doc_Event_Recall = list()
    doc_Event_f1score = list()
    ...
    for indoc in collection:
        ...
        doc = AllEventExtraction.getFeaturesTest(doc, targetEntityList, trained_classifier, Vectorizer)

        #Event Extraction Evaluation:
        total_doc_events = 0
        total_extracted_events = 0
        correctEventscounter = 0
        for tup in linksGold:
            if tup[0] == doc.get_doc_id():
                total_doc_events += 1
        for event in doc.Markables.EVENT_MENTION:
            total_extracted_events += 1
            event_t_id = event.get_token_anchor()[0].t_id
            event_sentence = int(utils.getToken(doc, event_t_id).sentence)
            event_text = utils.getEventText(doc, event).split('_')[0]          
            matches = [i for i, v in enumerate(linksGold) if (v[0], v[1], v[2].lower().split('_')[0]) == (doc.get_doc_id(), event_sentence, event_text)]
            if len(matches):
                correctEventscounter += 1
        if total_doc_events != 0:
            event_precision = float((float(correctEventscounter) / float(total_extracted_events))*100)
            event_recall = float((float(correctEventscounter) / float(total_doc_events))*100)
            if event_precision != 0 and event_recall != 0:
                event_f1score = float(float(2*event_precision*event_recall) / float(event_precision + event_recall))
        if event_precision != 0:
            doc_Event_Precision.append(event_precision)
        if event_recall != 0:
            doc_Event_Recall.append(event_recall)
        if event_f1score != 0 :
            doc_Event_f1score.append(event_f1score)
    ...
    total_precision = float(sum(doc_Event_Precision) / len(doc_Event_Precision))
    total_recall = float(sum(doc_Event_Recall) / len(doc_Event_Recall))
    total_f1score = float(sum(doc_Event_f1score) / len(doc_Event_f1score))

    print 'Event Precision:', total_precision
    print 'Event Recall:', total_recall
    print 'Event F1-score:', total_f1score
    ...
    for targetEntity in timeline.keys():
        ...
        for lst in timeline[targetEntity]:
            ...
            if eventClusterId == last_eventClusterId:
                f.write('\t' + str(doc.doc_id) + '-' + str(nsentence)+ '-' + str_event.encode('utf-8')) #+  "\t('" + str_timex + "')" 
            else:
                f.write('\n' + str(order) + '\t' + date_str + '\t' + str(doc_id) + '-' + str(nsentence)+ '-' + str_event.encode('utf-8')) #+  "\t('" + str_timex + "')" 
...
def run(test_corpus_list = [1], train_corpus_list = [0], link_model='structured_perceptron'):
    ...
    #Train classifier with the train corpus for event extraction:
    for train_corpus in train_corpus_list:
        Trained_Classifier_Events, Vectorizer = getTrainFeatures(train_corpus)
    ...
    for train_corpus in train_corpus_list:
        (collection_train, targetEntityList_train) = getCollection(train_corpus, Trained_Classifier_Events, Vectorizer)
    ...
    for corpus in test_corpus_list:
        (collection, targetEntityList) = getCollection(corpus, Trained_Classifier_Events, Vectorizer)
...
def crossval(train_corpus = 0, link_model='structured_perceptron'):
    ...
    args = []
    args += [("/Users/andreaslouka/Documents/UNIVERSITY/Thesis/andreas/cdeo/data/evaluation/corpus_%d/TimeLines_Gold_Standard/" %(corpus))]
    args += [("/Users/andreaslouka/Documents/UNIVERSITY/Thesis/andreas/cdeo/data/evaluation/corpus_%d/results/" %(corpus))]
    os.chdir('evaluation_tool')
